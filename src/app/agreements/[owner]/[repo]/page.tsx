...
export async function generateMetadata({
  ...
  return { title: `Sign CLA — ${owner}/${repo}` };
}
...
export default async function SigningPage({ params, searchParams }: PageProps) {
  const { owner, repo } = await params;
  ...
  // Build the current page URL for sign-in callback
  const callbackPath = `/agreements/${owner}/${repo}${rawReferrer ? `?referrer=${encodeURIComponent(rawReferrer)}` : ""}`;
